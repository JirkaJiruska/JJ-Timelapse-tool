...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        #####################################################################
        # END OF fill foe testing purposes
        #####################################################################
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.lineEdit_output_folder.setText(os.path.dirname(sys.argv[0]))
    ...
    def create_video(self):
        ...
        # check if there are only allowed characters in the path, otherwise cv2.VideoWriter will not create a file (known pathon3 issue)
        char = self.check_special_characters(output_dir, self.allowed_characters)

        print(output_dir)
        print(os.path.dirname(sys.argv[0]) + "/")
        print(char)
        print(output_dir[1:] != (os.path.dirname(sys.argv[0]) + "/")[1:])

        # during comparsion skip the first disk-letter, because it might be lower and upper as well
        if char and output_dir[1:] != (os.path.dirname(sys.argv[0]) + "/")[1:]:
            self.textBrowser.append(datetime.now().strftime("%H:%M:%S: ") + "Please don't use any special characters in output file, such as '{a}'.".format(a=char))
            return
        ...
        # create full name of the output folder, including path. Format:>> C:\User\....\folder\ + video + .avi
        if output_dir == os.path.dirname(sys.argv[0]):
            path = name
        else:
