...
class Ui_MainWindow(object):
    ...
    def load_files(self):
        ...
        previous_img = cv2.imdecode(np.fromfile(home_dir + os.listdir(home_dir)[0], dtype=np.uint8), cv2.IMREAD_UNCHANGED)
        ...
        for filename in os.listdir(home_dir):
            ...
            img = cv2.imdecode(np.fromfile(home_dir + filename, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
            size = img.shape
            if prev_size != size:
    ...
    def create_video(self):
        ...
        img = cv2.imdecode(np.fromfile(self.lineEdit_input_folder.text() + os.listdir(self.lineEdit_input_folder.text())[0], dtype=np.uint8), cv2.IMREAD_UNCHANGED)
        ...
        for image_name in img_names_list:
            ...
            img = cv2.imdecode(np.fromfile(image_name, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
